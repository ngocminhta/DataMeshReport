...
\begin{xltabular}{\textwidth}{||s|m|b|}
	...
	\cline{1-3}
	\multirow{3}{*}{\parbox{.13\textwidth}{Data product quantum}} & Data product structural components & Data product implemented as an architecture quantum that encapsulates all the structural components it needs to do its job—code, data, infrastructure specifications, and policies. \newline It is referred to in architectural discussions. It is used interchangeably with data products. \newline \textit{At the time of writing this is an experimental concept with custom implementations.} \\
	\cline{2-3}
	...
	\cline{2-3}
	& Data infrastructure (utility) plane & Platform plane providing low-level infrastructure resource management - compute, storage, identity, etc. \newline \textit{The infrastructure plane is mature and supported by many vendors with automated provisioning.} \\
	\cline{2-3}
	& Data product experience plane & Platform plan providing operations on a data product. \newline \textit{Custom implementation of services for data product experience plane, no reference implementation available.} \\
	\cline{2-3}
	& Mesh experience & Platform plane providing operations on the mesh of connected data products. \newline \textit{Custom implementations of discovery and search services have been implemented, but no reference implementation exists.} \\
	\cline{1-3}
	\multirow{4}{*}{\parbox{.13\textwidth}{Embedded Computational Policies \footnote{See figure \ref{LogicalComponents}}}} & Data product container & A mechanism to bundle all the structural components of a data product, deployed and run as a single unit with its sidecar. \newline \textit{At the time of writing this is an experimental concept with custom implementations.} \\
	\cline{2-3}
...
\begin{xltabular}{\textwidth}{||s|m|m|b|}
	...
	& \multirow{5}{*}{\parbox{.13\textwidth}{Data product experience}} & \verb*|/{dp}/status| & Checking the status of a data product. \\
	\cline{3-4}
	...
	\cline{2-4}
	& Data product experience plane & \verb*|/{dp}/| \verb*|controls| & Data products are registered with a unique global identifier and address, making them visible to governance. \\
\end{xltabular}
