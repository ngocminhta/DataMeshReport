...
Google Cloud allows organizations to use different execution engines optimized for different workloads and personas to run on top of the same data tiers. This is possible because of the complete separation of compute and storage on Google Cloud. Users can access data at their level of data access including SQL, Python, or more GUI-based methods. Google Cloud’s Data Catalog, along with Dataproc Metastore and the BigQuery Metastore, allow for accessing data wherever it is located with any query engine. The process is simple: onboard the data asset in a Dataplex lake, which triggers discovery and publishes the data in BigQuery and \textbf{Dataproc} Metastore. Tools such as Vertex AI notebooks, Spark running on Dataproc, BigQuery, or Serverless Spark can then be easily integrated into the workflow. With BigQuery and Serverless Spark, data engineers can spend all their time on the code and logic. They do not need to manage clusters or tune infrastructure. They submit SQL or PySpark jobs from their interface of choice and processing is auto-scaled to match the needs of the job.

Dataplex is a managed service that enables enterprises to group data containers from across their Google Cloud deployment and organize them in a hierarchy. It provides a single pane of glass to organize, govern, and index data in the Data Mesh. With Dataplex, you can curate, catalog, secure, integrate, and explore any type of data at any scale with an integrated experience. Dataplex enables quickly built lakes without needing to acquire and worry about different resources. It expands automatic data discovery and schema inference across different systems so that once you add a resource to Dataplex, it can be represented in the Data Mesh .
...
Data ownership and governance are crucial for the success of Data Mesh, and integrating governance with federated data sources is essential. Dataplex provides a data management and governance layer that allows you to efficiently organize your data across BigQuery and Cloud Storage while enforcing centralized governance rules. This empowers data administrators to establish and manage workspaces along with appropriate environment profiles, including compute parameters and libraries. They can also control user access and manage costs through a unified interface.

Dataplex simplifies access to notebooks for data scientists, offering one-click access and enabling them to discover notebooks through a repository linked to associated data. They can easily save and share notebooks as if they were sharing any other asset within the organization. Data analysts can leverage SQL Workspace for ad hoc analysis, eliminating the need for reliance on specific data processing environments. They can seamlessly use Presto, Hive, or BigQuery through a single interface, without the need to access multiple environments.

In addition, data access is made easy and uncomplicated. Google Cloud offers an integrated experience across all its data analytics services, providing virtual Lakehouse experiences. This is enhanced by a serverless notebook experience with serverless Spark, specifically designed for data science. All data stored in Cloud Storage is automatically made accessible for querying through OSS tools and BigQuery. Additionally, Data Catalog enables search and discovery capabilities across the entire data landscape.
